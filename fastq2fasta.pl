...
print "This script is used to convert fastq to fasta files.\n";
die "Usage:perl fastq2fasta.pl [the input file]\n" unless @ARGV>0;

my $fastq_file = $ARGV[0];
my $fasta_file = $fastq_file;
open TMP, "$fastq_file" || die "Cannot open fastq $fastq_file!\n";    
$fasta_file =~ s/fastq$|fq$/fasta/; 
open MYOUT, ">$fasta_file"|| die "Cannot open fasta $fasta_file"."_id!\n";
my ($li,$nl);
